...
// packages/core/dist/contract.js
...
// packages/core/dist/contract.js
var EvalCommandSchema = external_exports.object({
  kind: external_exports.literal("command"),
  command: external_exports.string().min(1),
  cwd: external_exports.string().optional()
});
var EvalGitChecksSchema = external_exports.object({
  kind: external_exports.literal("git-checks"),
  branch: external_exports.string().optional(),
  checks: external_exports.array(external_exports.string()).optional()
});
var EvalProofFileSchema = external_exports.object({
  kind: external_exports.literal("proof-file"),
  path: external_exports.string().min(1),
  contains: external_exports.string().optional()
});
var EvalSpecSchema = external_exports.discriminatedUnion("kind", [
  EvalCommandSchema,
  EvalGitChecksSchema,
  EvalProofFileSchema
]);
var ProofSpecSchema = external_exports.object({
...
var GoalContractSchema = external_exports.object({
  ...
  on_continue: external_exports.string().optional(),
...
function renderContinueMessage(contract, iteration, evalOutput) {
  const template = contract.on_continue ?? `Eval failed on iteration {{iteration}}. Fix remaining gaps and continue toward the goal.
...
Eval output:
{{eval_output}}`;
  return template.replaceAll("{{iteration}}", String(iteration)).replaceAll("{{goal}}", contract.goal).replaceAll("{{acceptance}}", contract.acceptance).replaceAll("{{eval_output}}", evalOutput);
}

// packages/core/dist/eval.js
...
function checkProofFreshness(workspace, proof, startedAtMs) {
  if (!proof?.path)
    return true;
  const proofPath = (0, import_node_path.resolve)(workspace, proof.path);
  if (!(0, import_node_fs2.existsSync)(proofPath))
    return false;
  try {
    const mtime = (0, import_node_fs2.statSync)(proofPath).mtimeMs;
    return mtime >= startedAtMs;
  } catch {
    return false;
  }
}
function evalCommand(spec, workspace) {
  const cwd = spec.cwd ? (0, import_node_path.resolve)(workspace, spec.cwd) : workspace;
  return runShell(spec.command, cwd);
}
function evalProofFile(spec, workspace) {
  const filePath = (0, import_node_path.resolve)(workspace, spec.path);
...
function evalGitChecks(spec, workspace) {
  const lines = [];
  let ok = true;
  if (spec.branch) {
    ...
    if (!branchResult.ok || branchResult.output.trim() !== spec.branch) {
      ok = false;
      lines.push(`Expected branch ${spec.branch}, got ${branchResult.output.trim() || "(unknown)"}`);
    }
  }
...
${result.output}`);
  ...
  if (checks.length === 0 && !spec.branch) {
    ...
    if (!status.ok && !(0, import_node_fs2.existsSync)((0, import_node_path.join)(workspace, ".git"))) {
      ok = false;
...
var EvalRunner = class {
  ...
  run(contract) {
    ...
    const output = proofFresh ? result.output : `${result.output}
...
function runStatePath(runsDir, runId) {
  return (0, import_node_path2.join)(runsDir, runId, "state.json");
}
function saveRunState(runsDir, state) {
  const path = runStatePath(runsDir, state.runId);
  (0, import_node_fs3.mkdirSync)((0, import_node_path2.dirname)(path), { recursive: true });
  (0, import_node_fs3.writeFileSync)(path, JSON.stringify(state, null, 2), "utf8");
}
function loadRunState(runsDir, runId) {
...
function findLatestRunId(runsDir) {
  if (!(0, import_node_fs3.existsSync)(runsDir))
    return void 0;
  const entries = (0, import_node_fs3.readdirSync)(runsDir).map((name) => {
    const full = (0, import_node_path2.join)(runsDir, name);
    try {
      return { name, mtime: (0, import_node_fs3.statSync)(full).mtimeMs };
    } catch {
      return null;
    }
...
function appendIteration(state, entry) {
  const next = {
    ...state,
...
function resolveHost(id) {
  ...
  const preference = ["outerCli", "outerApi", "inSessionGate", "proofOnly"];
  for (const mode of preference) {
    const match = [...adapters.values()].find((a) => a.capabilities.modes.includes(mode));
    if (match)
      return match;
  }
...
async function waitForIdle(adapter, handle, opts) {
...
async function runOrchestrator(options) {
  const { contract, contractPath, adapter, workspace, runsDir, pollIntervalMs = 500, iterationTimeoutMs = 30 * 60 * 1e3, signal } = options;
  const runId = options.runId ?? createRunId();
  const runDir = (0, import_node_path3.join)(runsDir, runId);
  (0, import_node_fs4.mkdirSync)(runDir, { recursive: true });
  if ((0, import_node_fs4.existsSync)(contractPath)) {
    (0, import_node_fs4.copyFileSync)(contractPath, (0, import_node_path3.join)(runDir, "goal.yaml"));
  } else {
    (0, import_node_fs4.writeFileSync)((0, import_node_path3.join)(runDir, "goal.yaml"), JSON.stringify(contract, null, 2));
  }
  ...
  const ctx = { runId, contract, workspace, runsDir };
  let handle;
  try {
    ...
    for (let iter = 1; iter <= contract.max_iterations; iter++) {
      ...
      state = appendIteration(state, {
        ...
        failureReason: evalResult.pass ? void 0 : "eval_fail"
      });
      saveRunState(runsDir, state);
      options.onState?.(state);
      if (evalResult.pass) {
    ...
    if (msg === "cancelled" || signal?.aborted) {
      if (handle)
        await adapter.cancel(handle).catch(() => void 0);
...
// packages/core/dist/in-session-gate.js
function evaluateInSessionGate(options) {
  ...
  if (iteration >= contract.max_iterations) {
    return {
      ...
      iteration,
      maxIterations: contract.max_iterations,
      followupMessage: void 0
    };
  }
  const template = options.onContinueTemplate ?? contract.on_continue ?? `Eval failed (iteration {{iteration}}/{{max}}). Do not stop. Continue working.
...
Eval output:
{{eval_output}}`;
  const followupMessage = template.replaceAll("{{iteration}}", String(iteration)).replaceAll("{{max}}", String(contract.max_iterations)).replaceAll("{{goal}}", contract.goal).replaceAll("{{acceptance}}", contract.acceptance).replaceAll("{{eval_output}}", evalResult.output);
  return {
    allowStop: false,
    followupMessage,
    eval: evalResult,
...
// adapters/cursor-ide/dist/index.js
var import_node_fs6 = require("node:fs");
var import_node_path5 = require("node:path");

// adapters/cursor-ide/dist/stop-hook.js
...
function evaluateCursorStop(opts) {
...
async function main() {
  ...
  const contractPath = process.env.GOAL_LOOP_CONTRACT || (0, import_node_path4.join)(workspace, ".goal-loop", "active-goal.yaml");
  if (!(0, import_node_fs5.existsSync)(contractPath)) {
    process.stdout.write("{}\n");
    return;
  }
  const stateFile = (0, import_node_path4.join)(workspace, ".goal-loop", "cursor-gate-state.json");
  let iteration = 1;
  if ((0, import_node_fs5.existsSync)(stateFile)) {
    try {
      iteration = JSON.parse((0, import_node_fs5.readFileSync)(stateFile, "utf8")).iteration ?? 0;
      iteration += 1;
    } catch {
      iteration = 1;
    }
  }
  (0, import_node_fs5.mkdirSync)((0, import_node_path4.dirname)(stateFile), { recursive: true });
  (0, import_node_fs5.writeFileSync)(stateFile, JSON.stringify({ iteration }, null, 2));
  const out = evaluateCursorStop({ workspace, contractPath, iteration });
  process.stdout.write(JSON.stringify(out) + "\n");
}
var isMain = process.argv[1] && (process.argv[1].endsWith("stop-hook.js") || process.argv[1].endsWith("stop-hook.ts"));
if (isMain) {
  main().catch((err) => {
    console.error(err);
    process.stdout.write("{}\n");
...
// adapters/cursor-ide/dist/index.js
var cursorIdeAdapter = {
  id: "cursor-ide",
  capabilities: { modes: ["inSessionGate"] },
  async start(ctx) {
    const statePath = (0, import_node_path5.join)(ctx.runsDir, ctx.runId, "cursor-ide.json");
    (0, import_node_fs6.mkdirSync)((0, import_node_path5.dirname)(statePath), { recursive: true });
    (0, import_node_fs6.writeFileSync)(statePath, JSON.stringify({ mode: "in-session", iteration: 0 }, null, 2));
    return {
...
function installCursorIdeHooks(workspace) {
  const hooksDir = (0, import_node_path5.join)(workspace, ".cursor");
  (0, import_node_fs6.mkdirSync)(hooksDir, { recursive: true });
  const hooksPath = (0, import_node_path5.join)(hooksDir, "hooks.json");
  const hookScript = (0, import_node_path5.join)(hooksDir, "goal-loop-stop.mjs");
  const hooks = {
    version: 1,
    hooks: {
      stop: [
  ...
  (0, import_node_fs6.writeFileSync)(hooksPath, JSON.stringify(hooks, null, 2), "utf8");
...
`;
  (0, import_node_fs6.writeFileSync)(hookScript, script, "utf8");
...
var claudeCodeAdapter = {
  ...
  capabilities: { modes: ["inSessionGate"] },
  async start(ctx) {
    const statePath = (0, import_node_path6.join)(ctx.runsDir, ctx.runId, "claude-code.json");
    (0, import_node_fs7.mkdirSync)((0, import_node_path6.dirname)(statePath), { recursive: true });
    (0, import_node_fs7.writeFileSync)(statePath, JSON.stringify({ mode: "in-session", iteration: 0 }, null, 2));
    return {
...
function installClaudeCodeHooks(workspace) {
  const hooksDir = (0, import_node_path6.join)(workspace, ".claude", "hooks");
  (0, import_node_fs7.mkdirSync)(hooksDir, { recursive: true });
  const scriptPath = (0, import_node_path6.join)(hooksDir, "goal-loop-stop.mjs");
  const settingsPath = (0, import_node_path6.join)(workspace, ".claude", "settings.json");
  const script = `#!/usr/bin/env node
/**
...
`;
  (0, import_node_fs7.writeFileSync)(scriptPath, script, "utf8");
  let settings = {};
  if ((0, import_node_fs7.existsSync)(settingsPath)) {
    try {
      settings = JSON.parse((0, import_node_fs7.readFileSync)(settingsPath, "utf8"));
    } catch {
      settings = {};
    }
  }
  ...
  (0, import_node_fs7.mkdirSync)((0, import_node_path6.dirname)(settingsPath), { recursive: true });
  (0, import_node_fs7.writeFileSync)(settingsPath, JSON.stringify(settings, null, 2), "utf8");
  return [scriptPath, settingsPath];
}

// adapters/generic-shell/dist/index.js
...
function runCommand(meta, prompt) {
  return new Promise((resolve4) => {
    ...
    const child = (0, import_node_child_process2.spawn)(meta.command, args, {
      cwd: meta.workspace,
      shell: true,
      env: process.env,
    ...
    child.on("close", (code) => {
      ...
      const logDir = (0, import_node_path7.join)(meta.runsDir, meta.runId);
      (0, import_node_fs8.mkdirSync)(logDir, { recursive: true });
      (0, import_node_fs8.writeFileSync)((0, import_node_path7.join)(logDir, `shell-iter-${Date.now()}.log`), meta.output, "utf8");
      resolve4();
    });
  });
}
function createGenericShellAdapter(defaults) {
  return {
    id: "generic-shell",
    capabilities: { modes: ["outerCli"] },
    async start(ctx) {
      const command = ctx.contract.command ?? defaults?.command ?? "echo";
      const args = ctx.contract.args ?? defaults?.args ?? (command === "echo" || command === "Write-Output" ? ["{{prompt}}"] : ["{{prompt}}"]);
      const handle = {
        id: `shell-${ctx.runId}`,
    ...
    async continue(handle, delta) {
      const meta = sessions.get(handle.id);
      if (!meta)
        throw new Error(`Unknown session: ${handle.id}`);
    ...
    async collectEvidence(handle) {
      const meta = sessions.get(handle.id);
      return {
        artifacts: meta ? [(0, import_node_path7.join)(meta.runsDir, meta.runId)] : [],
        metadata: { lastPrompt: meta?.lastPrompt }
      };
    }
  };
}
var genericShellAdapter = createGenericShellAdapter();

// adapters/cursor-cloud/dist/index.js
var sessions2 = /* @__PURE__ */ new Map();
var MISSING_KEY_ERROR = "CURSOR_API_KEY is required for cursor-cloud adapter. Set CURSOR_API_KEY (or CURSOR_CLOUD_API_KEY). Optional: CURSOR_API_BASE (default https://api.cursor.com).";
function getApiBase() {
...
function isCursorCloudConfigured() {
...
function buildInitialPrompt2(ctx) {
  return [
    ctx.contract.goal,
    "",
...
var cursorCloudAdapter = {
  ...
  async start(ctx) {
    ...
    const body = {
      prompt: { text: buildInitialPrompt2(ctx) }
    };
    ...
    sessions2.set(agentId, { agentId, status: "running", output: "" });
    return handle;
  },
  async continue(handle, delta) {
    if (!isCursorCloudConfigured()) {
    ...
    const session = sessions2.get(handle.id);
    if (session) {
  ...
  async collectEvidence(handle) {
    ...
    return {
      branch: data.branch ?? target?.branchName,
      prUrl: data.prUrl ?? target?.prUrl,
      metadata: data
    };
  }
};
function mapCursorStatus(apiStatus) {
  ...
  switch (known) {
    ...
    case "stopped":
      return "exited";
    default: {
      const _exhaustive = known;
      return _exhaustive;
    }
  }
}
function asCursorApiStatus(raw) {
  const known = [
    "RUNNING",
    "running",
    "FINISHED",
...
// adapters/devin/dist/index.js
var sessions3 = /* @__PURE__ */ new Map();
var MISSING_KEY_ERROR2 = "DEVIN_API_KEY is required for devin adapter. Set DEVIN_API_KEY and DEVIN_ORG_ID. Optional: DEVIN_API_BASE (default https://api.devin.ai/v3).";
...
function requireConfig() {
  ...
  if (!apiKey) {
    throw new Error(MISSING_KEY_ERROR2);
  }
  const orgId = getOrgId();
  if (!orgId) {
    throw new Error("DEVIN_ORG_ID is required for devin adapter. Find it on Settings \u2192 Service Users in the Devin dashboard.");
  }
  return { apiKey, orgId };
}
function isDevinConfigured() {
  return getApiKey2() !== null && getOrgId() !== null;
}
async function devinRequest(method, path, body) {
  const { apiKey } = requireConfig();
  const res = await fetch(`${getApiBase2()}${path}`, {
    method,
    headers: {
...
function orgSessionsPath(orgId, sessionId) {
  const base = `/organizations/${orgId}/sessions`;
  return sessionId ? `${base}/${sessionId}` : base;
}
function buildInitialPrompt3(ctx) {
  return [
    ctx.contract.goal,
    "",
    `Acceptance criteria: ${ctx.contract.acceptance}`
...
var devinAdapter = {
  ...
  async continue(handle, delta) {
    ...
    const session = sessions3.get(handle.id);
    if (session) {
      session.status = "running";
    }
  },
  async poll(handle) {
    if (!isDevinConfigured()) {
      return { status: "error", error: MISSING_KEY_ERROR2 };
    }
    const { orgId } = requireConfig();
  ...
  async cancel(handle) {
    ...
    sessions3.delete(handle.id);
  },
  async collectEvidence(handle) {
    if (!isDevinConfigured()) {
      return { metadata: { error: MISSING_KEY_ERROR2 } };
    }
    const { orgId } = requireConfig();
    const data = await devinRequest("GET", orgSessionsPath(orgId, handle.id));
    return {
      branch: data.branch,
      prUrl: data.pull_request_url ?? data.pr_url,
      metadata: data
    };
  }
};
function mapDevinStatus(status) {
...
// adapters/codex/dist/index.js
var shell = createGenericShellAdapter({
  command: "codex",
  args: ["exec", "{{prompt}}"]
});
var codexAdapter = {
...
// packages/cli/src/register-adapters.ts
...
function resolveWorkspace(path) {
  return (0, import_node_path8.resolve)(path ?? process.cwd());
}
function resolveRunsDir(workspace, runsDir) {
  return runsDir ? (0, import_node_path8.resolve)(runsDir) : (0, import_node_path8.join)(workspace, ".goal-loop", "runs");
}
function goalLoopDir(workspace) {
  return (0, import_node_path8.join)(workspace, ".goal-loop");
}
...
function moduleDir() {
  ...
  if (argvPath) return (0, import_node_path9.dirname)(argvPath);
  return process.cwd();
}
function listSkillSourceCandidates() {
  const here = moduleDir();
  return [
    (0, import_node_path9.join)(here, "..", "skills", "goal-loop"),
    (0, import_node_path9.join)(here, "skills", "goal-loop"),
    (0, import_node_path9.join)(here, "..", "..", "..", "skills", "goal-loop"),
    (0, import_node_path9.join)(here, "..", "..", "..", "plugins", "goal-loop", "skills", "goal-loop"),
    (0, import_node_path9.join)(here, "..", "plugins", "goal-loop", "skills", "goal-loop"),
    (0, import_node_path9.join)(process.cwd(), "skills", "goal-loop"),
    (0, import_node_path9.join)(process.cwd(), "packages", "goal-loop", "skills", "goal-loop")
  ];
}
function resolveSkillSourceDir() {
  for (const candidate of listSkillSourceCandidates()) {
    if ((0, import_node_fs9.existsSync)((0, import_node_path9.join)(candidate, "SKILL.md"))) {
      return candidate;
    }
  }
  throw new Error(
...
function skillDestDir(workspace, target, global) {
  const base = target === "cursor" ? global ? (0, import_node_path9.join)((0, import_node_os.homedir)(), ".cursor", "skills") : (0, import_node_path9.join)(workspace, ".cursor", "skills") : global ? (0, import_node_path9.join)((0, import_node_os.homedir)(), ".claude", "skills") : (0, import_node_path9.join)(workspace, ".claude", "skills");
  return (0, import_node_path9.join)(base, "goal-loop");
}
function copySkillDir(sourceDir, destDir) {
  (0, import_node_fs9.mkdirSync)(destDir, { recursive: true });
  for (const entry of (0, import_node_fs9.readdirSync)(sourceDir, { withFileTypes: true })) {
    const from = (0, import_node_path9.join)(sourceDir, entry.name);
    const to = (0, import_node_path9.join)(destDir, entry.name);
    if (entry.isDirectory()) {
      copySkillDir(from, to);
    } else {
      (0, import_node_fs9.cpSync)(from, to);
    }
  }
}
...
// packages/cli/src/commands.ts
function loadRunStateOrLatest(workspace, runId, runsDir) {
...
function applyCommandOverride(contract, commandParts) {
...
async function runCommand2(options) {
  ...
  const contractPath = (0, import_node_path10.isAbsolute)(options.goal) ? (0, import_node_path10.resolve)(options.goal) : (0, import_node_path10.resolve)(workspace, options.goal);
  ...
  const hostId = options.host === "auto" ? contract.host ?? "auto" : options.host;
...
function statusCommand(workspace, runId, runsDir) {
...
async function cancelCommand(workspace, runId, runsDir) {
  registerAllAdapters();
  const ws = resolveWorkspace(workspace);
  ...
  await adapter.cancel({ id: state.runId, adapterId: state.host }).catch(() => void 0);
  const cancelled = markCancelled(state);
  saveRunState(dir, cancelled);
  console.log(`Cancelled run ${state.runId}`);
}
var ACTIVE_GOAL_TEMPLATE = `goal: Describe your goal
...
host: auto
`;
function scaffoldGoalLoop(workspace) {
  const base = goalLoopDir(workspace);
  (0, import_node_fs10.mkdirSync)((0, import_node_path10.join)(base, "runs"), { recursive: true });
  const activeGoalPath = (0, import_node_path10.join)(base, "active-goal.yaml");
  if (!(0, import_node_fs10.existsSync)(activeGoalPath)) {
    (0, import_node_fs10.writeFileSync)(activeGoalPath, ACTIVE_GOAL_TEMPLATE, "utf8");
    console.log(`Created ${activeGoalPath}`);
  } else {
    console.log(`Active goal already exists: ${activeGoalPath}`);
  }
}
function initCommand(host, workspace, options) {
  const ws = resolveWorkspace(workspace);
  scaffoldGoalLoop(ws);
  switch (host) {
    case "skill": {
...
function checkAdapterReady(adapter) {
  switch (adapter.id) {
    case "cursor-cloud":
      return Boolean(
...
function doctorCommand() {
  registerAllAdapters();
  const adapters2 = listAdapters().sort((a, b) => a.id.localeCompare(b.id));
  ...
  console.log("\nEnvironment checks:");
  const cursorKey = process.env.CURSOR_API_KEY ?? process.env.CURSOR_CLOUD_API_KEY;
  ...
  const cloudAdapters = adapters2.filter(
...
program2.name("goal-loop").description("Harness-agnostic goal loop with eval-gated stop").version("0.1.0");
program2.command("run").description("Run the goal loop orchestrator").requiredOption("--goal <path>", "Path to goal.yaml contract").option("--host <id>", "Host adapter id or auto", "auto").option("--workspace <path>", "Workspace directory", process.cwd()).option("--runs-dir <path>", "Directory for run state (default: .goal-loop/runs)").option(
  "--command <parts...>",
  "Override contract command (first token is executable, rest are args)"
).action(
  async (opts) => {
    await runCommand2(opts);
  }
);
program2.command("status").description("Show run status").argument("[runId]", "Run id (default: latest)").option("--workspace <path>", "Workspace directory", process.cwd()).option("--runs-dir <path>", "Runs directory override").action(
  (runId, opts) => {
    statusCommand(opts.workspace, runId, opts.runsDir);
  }
);
program2.command("cancel").description("Cancel an active run").argument("[runId]", "Run id (default: latest)").option("--workspace <path>", "Workspace directory", process.cwd()).option("--runs-dir <path>", "Runs directory override").action(
  async (runId, opts) => {
    await cancelCommand(opts.workspace, runId, opts.runsDir);
  }
);
program2.command("init").description("Install agent skill (default) or host hooks").option(
  "--host <id>",
  "Host: skill | cursor-skill | claude-skill | cursor-ide | claude-code | \u2026",
  "skill"
).option("--workspace <path>", "Workspace directory", process.cwd()).option("--global", "Install skill to user home (~/.cursor or ~/.claude)").action((opts) => {
  initCommand(opts.host, opts.workspace, { global: opts.global });
});
program2.command("doctor").description("Check capability matrix and environment").action(() => {
  doctorCommand();
});
program2.parse();
